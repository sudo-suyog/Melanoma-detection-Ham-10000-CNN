...
# Define a function to augment images
...
def create_alexnet(hp):
    ...
    model = Sequential()
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2), input_shape=(Image_Size, Image_Size, 3)))
    model.add(Conv2D(hp.Int('conv1_filters', min_value=32, max_value=256, step=32),
                     kernel_size=(11, 11),
                     strides=(4, 4),
                     activation='relu'))
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    model.add(Conv2D(hp.Int('conv2_filters', min_value=32, max_value=256, step=32),
                     kernel_size=(5, 5),
                     padding='same',
                     activation='relu'))
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    model.add(Conv2D(hp.Int('conv3_filters', min_value=32, max_value=256, step=32),
                     kernel_size=(3, 3),
                     padding='same',
                     activation='relu'))
    model.add(Conv2D(hp.Int('conv4_filters', min_value=32, max_value=256, step=32),
                     kernel_size=(3, 3),
                     padding='same',
                     activation='relu'))
    model.add(Conv2D(hp.Int('conv5_filters', min_value=32, max_value=256, step=32),
                     kernel_size=(3, 3),
                     padding='same',
                     activation='relu'))
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    model.add(Flatten())
    model.add(Dense(hp.Int('dense1_units', min_value=32, max_value=1024, step=32),
                    activation='relu'))
    model.add(Dropout(0.5))
    model.add(Dense(hp.Int('dense2_units', min_value=32, max_value=1024, step=32),
                    activation='relu'))
    model.add(Dropout(0.5))
...
tuner.search(X_train_augmented, y_train_augmented, epochs=20, validation_split=0.2)
...
history = best_model.fit(X_train_augmented, y_train_augmented, epochs=25, validation_split=0.2)
